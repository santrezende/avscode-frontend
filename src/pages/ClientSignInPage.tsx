...
function ClientSignInPage() {
  ...
  const handlelicensePlateInput = (event: ChangeEvent<HTMLInputElement>) =>
    setLicensePlatePage(event.target.value);

  const [cpfPage, setCpfPage] = useState("");
  const handleCpfInput = (event: ChangeEvent<HTMLInputElement>) =>
    setCpfPage(event.target.value);
  ...
  const handleSignInClick = async () => {
    try {
      const response = await api.post(
        `/vehicles/${licensePlatePage.toUpperCase()}`,
        { licensePlate: licensePlatePage.toUpperCase(), cpf: cpfPage },
      );
